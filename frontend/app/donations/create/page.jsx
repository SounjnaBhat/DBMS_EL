...
export default function CreateDonationPage() {
  ...
  const { location: gpsLocation, address: gpsAddress, loading: gpsLoading, error: gpsError, getCurrentLocation } = useLocation();
  ...
  // Auto-fill location if GPS found
  useEffect(() => {
    if (gpsAddress) {
      setFormValues(prev => ({ ...prev, location: gpsAddress }));
    } else if (gpsLocation) {
       setFormValues(prev => ({ ...prev, location: "Locating address..." }));
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (gpsLocation) {
        formData.set("latitude", gpsLocation.latitude);
        formData.set("longitude", gpsLocation.longitude);
    }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       ...
                       <div>
                          <FormInput
                            ...
                            type="datetime-local"
                            required
                            defaultValue={defaults?.prepared_at ? toDateTimeLocal(defaults.prepared_at) : toDateTimeLocal(new Date())}
                          />
                       </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <button
                        ...
                        disabled={gpsLoading}
                        className="absolute right-2 top-8 rounded-lg bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-700 hover:bg-teal-100 transition-colors flex items-center gap-1.5"
                      >
                        {gpsLoading ? (
                        ...
                        {gpsLocation ? "GPS Active" : "Get Location"}
                      </button>
