        ...
        {roles.map((role) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-start p-3 border rounded-xl transition-all duration-200 text-left h-full ${isSelected
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === 'receiver' && !formData.address) {
      setNotification({ type: 'error', message: 'Address is required for Receivers.' });
      return;
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB] font-sans text-slate-900 selection:bg-teal-100 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="w-full max-w-lg bg-white p-8 sm:p-10 rounded-2xl shadow-xl border border-slate-100 relative z-10">
          ...
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            {role === 'receiver' && (
              <InputField
                label="Organization Address / Location"
                type="text"
                placeholder="123 NGO Street, City, Country"
                icon={MapPin}
            ...
            {
              notification && (
                <div className={`mt-2 rounded-lg border px-3 py-3 text-sm flex items-center gap-2 ${notification.type === 'error'
